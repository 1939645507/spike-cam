...
from utils.ae_parameters import MODEL_PATH, PLOT_PATH
...
def build_autoencoder_model_bundle(
    ae_type,
    input_dim,
    ae_layers,
    code_size,
    ...
    """Build an autoencoder model from the external repo without training it.

    中文：
    这个 helper 用于把原仓库中的模型构建逻辑抽出来，方便主项目做：

    - 训练后保存权重
    - 后续重新构图并加载权重
    - 不必每次重新训练

    目前主要覆盖本项目主线会用到的前馈 AE 类型。
    """

    resolved_layers = np.asarray(ae_layers, dtype=int)
    resolved_loss = loss_function

    if ae_type == "shallow":
        resolved_layers = np.asarray([40, 20, 10], dtype=int)
        model_wrapper = AutoencoderModel(
            input_size=input_dim,
            encoder_layer_sizes=resolved_layers,
            decoder_layer_sizes=resolved_layers,
            code_size=code_size,
        ...
        raise ValueError(
            "Artifact train/load currently supports ae_type in "
            "{'normal', 'shallow', 'contractive'}"
        )

    encoder_model, autoencoder_model = model_wrapper.return_encoder()
    return {
        "model_wrapper": model_wrapper,
        "encoder": encoder_model,
        "autoencoder": autoencoder_model,
        "resolved_layers": resolved_layers.astype(int).tolist(),
        "resolved_loss": resolved_loss,
    }


def train_autoencoder_for_spikes(
    spikes,
    ae_type,
    ae_layers,
    code_size,
    ...
    *,
    ...
    """Train one external AE on a concrete spike matrix and return models.

    中文：
    这个函数是给主项目调用的正式训练接口：

    - 输入已经准备好的 spike waveform 矩阵
    - 可选择是否在这里内部做 scale
    - 返回 encoder / autoencoder / features / history
    """

    spikes = np.asarray(spikes, dtype=np.float32)
    training_spikes = choose_scale(spikes, scale) if apply_scale else spikes

    bundle = build_autoencoder_model_bundle(
        ae_type=ae_type,
        input_dim=training_spikes.shape[1],
        ae_layers=ae_layers,
        code_size=code_size,
        output_activation=output_activation,
        loss_function=loss_function,
    ...
    if initial_weights_path is not None:
        autoencoder_model.load_weights(initial_weights_path)
    ...
    features = get_codes(training_spikes, encoder_model)
    ...
    return {
        ...
        "encoder": encoder_model,
        "autoencoder": autoencoder_model,
        "training_data": np.asarray(training_spikes, dtype=np.float32),
        "history": history_payload,
        "resolved_layers": bundle["resolved_layers"],
        "resolved_loss": bundle["resolved_loss"],
    }


def run_autoencoder(data_type, simulation_number, data, labels, gt_labels, index, ae_type, ae_layers, code_size, output_activation, loss_function, scale, shuff=True, noNoise=False, nr_epochs=100,
                    doPlot=False, savePlot=False, saveWeights=False, dropout=0.0, weight_init='glorot_uniform', learning_rate=0.001, verbose=1):
    if data_type == "real":
    ...
    if ae_type == "shallow":
        ...
        if saveWeights:
            autoencoder.save_weights(MODEL_PATH + f'autoencoder_cs{code_size}_scale{scale}_oa-{output_activation}_ls-{loss_function}_sim{simulation_number}_e{nr_epochs}')
    ...
    if ae_type == "contractive":
        ...
        try:
            autoencoder_features = encoder.predict(spikes, verbose=0)
        except TypeError:
        ...
        if saveWeights:
            autoencoder.save_weights(MODEL_PATH + f'lstm_cs{code_size}_scale{scale}_oa-{output_activation}_ls-{loss_function}_sim{simulation_number}_e{nr_epochs}')
        # autoencoder.load_weights('./feature_extraction/autoencoder/weights/autoencoder_cs{code_size}_oa-tanh_sim4_e100')
        encoder, autoencoder = autoencoder.return_encoder()
