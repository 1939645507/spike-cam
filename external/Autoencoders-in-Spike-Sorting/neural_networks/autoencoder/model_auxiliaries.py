...
from neural_networks.softplusplus_activation import softplusplus
...
def create_code_numpy(spike, encoder):
    try:
        return encoder.predict(spike.reshape(1, -1), verbose=0)[0]
    except TypeError:
        return encoder.predict(spike.reshape(1, -1))[0]


def get_codes(training_data, encoder):
    # 中文：原仓库这里逐条 spike 调一次 predict，会非常慢，
    # 在本实验平台里统一改成 batch predict。
    try:
        return encoder.predict(training_data, verbose=0)
    except TypeError:
...
def create_reconstruction_numpy(spike, autoencoder):
    try:
        return autoencoder.predict(spike.reshape(1, -1), verbose=0)[0]
    except TypeError:
...
def get_reconstructions(training_data, autoencoder):
    try:
        return autoencoder.predict(training_data, verbose=0)
    except TypeError:
